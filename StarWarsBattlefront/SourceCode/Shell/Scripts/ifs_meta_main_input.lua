...
--		IFText_fnSetString(Metagame_Popup_PickWinner.showodds,format("Odds: %d %%",Odds))
--		if(Odds > r) then
...
function metagame_input_Leftstick(this, iJoystick, XBias, YBias, bFromAI)
	...
	if(abs(x) < abs(XBias)) then
		x = XBias
	end

	if(abs(y) < abs(YBias)) then
		y = YBias
	end
	...
	local rndPick=random(getn(gMetaAttackableList))
...
function metagame_input_BonusLeftstick(this, iJoystick, XBias, YBias, bFromAI)
	...
	if(bFromAI) then
		...
		if(numSelectable>1) then
			local rndPick = random(numSelectable)
			local cnt=1
			for k,v in metagame_state.planets do
				if((not v.destroyed) and (v.pickorder <= NumPlanets)) then
					if( (v.owner1 == v.owner2) and (TeamNumber == v.owner1) ) then				
...
function metagame_input_GeneralLeft(this,iJoystick,bFromAI)
    ...
    print(format("team%d",metagame_state_local.mJoystickTeams[iJoystick+1]))
    
    
	local CurTeam = metagame_state[format("team%d",metagame_state_local.mJoystickTeams[iJoystick+1])]
