...
ifs_sp_briefing = NewIFShellScreen {
	...
	bg_texture = HackBGTextureOnPCOnly("generic2_screen"),
...
--		print("ifs_sp_briefing.Enter. this=",this," iProgress=", this.iProgress)
		...
		elseif( bFwd or this.returningFromUnlockable ) then
			...
			if(ScriptCB_IsSPStateSaved()) then
				this.iCampaignNum,bOnLastMission,this.iNumLastMission = ScriptCB_IsSPStateSaved()
...
--				print("Calculated campaign #",this.iCampaignNum)
			...
			if(iMissionSuccess >= 0) then
				...
				-- did we win?
				if(abs(PlayerSide - iMissionSuccess) < 0.1) then
			...
			this.iMaxMissions = getn(gCurCampaign)
			-- Build list of textures for the missions as well
			local i
			local k,v
			for i=1,this.iMaxMissions do
				gCurCampaign[i].texture = "map_" .. strsub(gCurCampaign[i].mapluafile,1,4)
				gCurCampaign[i].Mesh = "planet_" .. strsub(gCurCampaign[i].mapluafile,1,3)

				gCurCampaign[i].PlanetStr = "???"
				local Planet3 = strupper(strsub(gCurCampaign[i].mapluafile,1,3))
			...
			if(this.wonLastMission) then
				...
				local PlayerSide = Selection.side or 1
				this.iNumLastMission = min(this.iNumLastMission + 1, this.iMaxMissions)
				--did we win the last mission thats unlocked?			
				if(bOnLastMission) then
					this.iProgress = min(this.iProgress + 1, this.iMaxMissions)
...
--		print( "Curbutton = ",this.CurButton)
		...
		elseif (this.CurButton == "launch" or gPlatformStr ~= "PC" ) then
			...
			if(this.bArchiveMode) then
				...
				ScriptCB_SaveSPState(1,this.iCampaignNum,this.iSelected == this.iProgress,this.iSelected)
