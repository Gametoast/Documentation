...
function metagame_GetNumPlanets()
	return getn(metagame_state.PlanetPositions)
end
...
function metagame_util_fnGetValidMapName(MapStr)
	...
	local MapWOTeamStr = strsub(MapStr,1,4) -- left 4 characters of MapStr (e.g. "YAV1")
	local PlanetStr = strsub(MapStr,1,3) -- just the planet name (e.g. "YAV")
	local TypeChar = strsub(MapStr,4,4) -- '1' or '2'
	local SideChar = strsub(MapStr,5,5) -- 'A' 'C' 'I' or 'R'
	local i
...
-- 	end
	...
	for i = 1,getn(MapList) do
		if(TypeChar == strsub(MapList[i].mapluafile,4,4)) then
	...
	for i = 1,getn(MapList) do
		if(PlanetStr == strupper(strsub(MapList[i].mapluafile,1,3))) then
			return MapList[i].mapluafile .. SideChar -- right suffix
		end -- planet name is correct
	end
	...
	for i = 1,getn(MapList) do
		if((MapList[i].side_a) and (MapList[i].side_c)) then
			return strsub(MapList[i].mapluafile,1,4) .. SideChar -- left 4 characters of MapStr
...
--	metagame_ai:fnMakeNextMove(metagame_state,fDt)
	...
	-- Spice up random # generator by constantly reading off it
	local r = random(100)
