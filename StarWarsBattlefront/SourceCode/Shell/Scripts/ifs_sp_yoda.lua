...
--	IFObj_fnSetVis(this.Helptext_Accept,nil)
	...
	local lang, langEnum = ScriptCB_GetLanguage();
	local langShort = strlower(strsub(lang, 1, 3))
	...
	if (localized) then
		...
		if(gPlatformStr == "XBox") then
			ScriptCB_CloseMovie()
			local movie_pre = strsub( movieName, 1, 3 )
...
--			print("ifs_sp_yoda.popAfterExitMovie")
		...
		if(ScriptCB_GetLanguage() ~= "english") then
			ScriptCB_IFText_SetLeading(this.Box.description.cpointer,-2)
		end
		...
		if( bFwd ) then
			if(gPlatformStr == "XBox") then
				ScriptCB_CloseMovie();
				if(Selection) then
					print( "+++Selection.movie", Selection.movie, strsub(Selection.movie, 5, 7) )
					if( strsub(Selection.movie, 5, 7) == "fly" ) then
