...
function metagame_display_fnUpdateMainTitle(this)
	...
	if(gMetagameCurrentPage == 1) then
		if(not metagame_state.applyingbonus) then
			local CurTeamName = ScriptCB_getlocalizestr(metagame_state[format("team%d",metagame_state.pickteam)].teamname)
			...
			local tname = metagame_state[format("team%d",metagame_state.pickteam)].teamname
	...
	elseif (gMetagameCurrentPage == 4) then
		local CurTeamName = ScriptCB_getlocalizestr(metagame_state[format("team%d",3-metagame_state.pickteam)].teamname)
		-- Generate this with ScriptCB_getlocalizestr() or ScriptCB_tounicode()
		local NewTitle = ScriptCB_getlocalizestr("ifs.meta.Bonus.select")
...
function metagame_display_PlanetPlayVO(teamname, mapluafile)
    ...
    if (gPrevMetagamePage == gMetagameCurrentPage) then
	    ...
	    if (gMetagameCurrentPage == 3) or (gMetagameCurrentPage == 4) then
            mapname    = strlower(strsub(mapluafile, 1, 3))
		    actiontype = "_bonus"
	    else
...
function metagame_display_fnSetMapPreviews(this)
	...
	ScriptCB_IFModel_SetDepth( this.AttackItems.ModelC.cpointer, 0.12 )
	if(getn(gMetaAttackableList) >= 1) then
...
--		gButtonWindow_fnSetText(this.AttackBonusName, Selection.BonusName )
		...
		local CurTeam = metagame_state[format("team%d",metagame_state.pickteam)]
		local origMapName = MapName
		MapName = MapName .. strupper(CurTeam.char)
		MapName = strsub(MapName,1,4)
		...
		local Map12Char = strsub(MapName,4,4) -- 1 or 2
		local MapExtension = "MapName" .. Map12Char
	--	print("TargetName = ", TargetPlanet[MapExtension])
--		local LocalizedMapName = ScriptCB_getlocalizestr(TargetPlanet[MapExtension])
--		IFText_fnSetString(this.AttackMapName,TargetPlanet[MapExtension])

		--IFMapPreview_fnSetTexture(this.AttackMapImage, "map_" .. strsub(origMapName,1,4))
		--IFImage_fnSetTexture( this.AttackMapImage, "map_" .. strsub(origMapName,1,4) )

--	IFMapPreview_fnSetTexture(this.CenterItems.Map,"map_" .. strsub(metagame_state.TrueMapName,1,4))
...
function metagame_display_fnSetBonusAttackMap(this, dontPlayVO)
	...
	if( (planet_name == nil) or (planet_name == "none") ) then
...
function metagame_display_fnColorPlanetName( dest, Planet, model )
	...
	if(gMetagameCurrentPage==1) then
		...
		for Idx = 1,getn(gMetaAttackableList) do
	...
	-- Team of first owner
	local Team = metagame_state[format("team%d",Planet.owner1)]
...
function metagame_display_fnUpdateGalaxyPlanets(this)
	...
	local NumNormalPlanets = metagame_GetNumPlanets()
	...
	for k,v in metagame_state.planets do
		...
		if(i <= NumNormalPlanets) then
			...
			if((planetVotes>0) and (gMetagameCurrentPage==1)) then
				...
				IFText_fnSetString(dest[i].numVotes,format("%d",planetVotes))
			else
				...
				if((numAttackBonusVotes > 0) and (gMetagameCurrentPage == 3)) then
					IFObj_fnSetVis(dest[i].numVotes, 1)
					IFText_fnSetString(dest[i].numVotes,format("%d",numAttackBonusVotes))
				elseif((numDefendBonusVotes > 0) and (gMetagameCurrentPage == 4)) then
					IFObj_fnSetVis(dest[i].numVotes, 1)
					IFText_fnSetString(dest[i].numVotes,format("%d",numDefendBonusVotes))
				else
			...
			ScriptCB_IFModel_SetDepth( dest[i].hexagon.cpointer, dest[i].hexagon.depth )
			IFObj_fnSetAlpha( dest[i].hexagon, 0.5 )

			ScriptCB_IFModel_SetDepth( dest[i].cursor.cpointer, dest[i].cursor.depth )
			
			-- set the mesh for the planet model
			local Mesh = "planet_" .. v.MapName
			IFModel_fnSetMsh( dest[i].model, Mesh )
			ScriptCB_IFModel_SetDepth( dest[i].model.cpointer, dest[i].model.depth )
			Mesh = "bonusplanet_" .. v.MapName
			IFModel_fnSetMsh( dest[i].bonus, Mesh )
			ScriptCB_IFModel_SetDepth( dest[i].bonus.cpointer, dest[i].bonus.depth )			
			...
			if((k ~= pickPlanet)) then --  or (not metagame_state.applyingbonus)
				...
				OwnerIconAlpha = 0.9
				--ScriptCB_IFModel_SetDepth( dest[i].model.cpointer, dest[i].model.depth * 0.75 )
			end
