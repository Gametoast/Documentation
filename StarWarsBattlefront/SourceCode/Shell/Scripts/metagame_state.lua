	...
	for k,v in this.planets do
		if(not v.destroyed) then
			...
			if(v.faction_planet) then
				...
				-- goes up by 2. Else 1.
				if(abs(v.owner1 - v.owner2) < 0.1) then
					v.charge = v.charge + 2
				else
					v.charge = v.charge + 1
				end
			...
			-- Clamp charge to max
			v.charge = min(v.charge,v.chargemax)
		end -- not destroyed
...
function metagame_state_fnCouldPickSelected(this,which)
	...
	if(bCanPick) then
		...
		-- one available
		local CurTeam = this[format("team%d",this.pickteam)]
...
--		idx1 = random(NumNeutral)
--		idx2 = random(NumNeutral)
...
function metagame_state_fnAddAttackable(k,v,MapIdx)
	local Idx = getn(gMetaAttackableList) + 1
...
function metagame_state_fnGetAttackTeam(this)
    return metagame_state[format("team%d", metagame_state.pickteam)]
end
...
function metagame_state_fnGetDefendTeam(this)
    return metagame_state[format("team%d", metagame_state_fnGetDefendTeamIndex(this))]
end
