...
function ifs_meta_main_fnSetPage(this,iNewPage)
	...
	if (gMetagameCurrentPage == 1) then
		...
		if( this.iMapSelectIdx > getn(gMetaAttackableList) ) then
			this.iMapSelectIdx = 1
		end
		...
		if(not this.iMapSelectIdx) then
			if(getn(gMetaAttackableList)>0) then
...
--		if(iLastPage == 1) then
--			metagame_state_fnDetermineAttackable(metagame_state)
--			this.iMapSelectIdx = getn(gMetaAttackableList)
--
--			for Idx = 1,getn(gMetaAttackableList) do
...
--		end
		...
		metagame_state.CurTime = max(metagame_state.CurTime,gTimeOutMax)
	...
	elseif ((gMetagameCurrentPage == 3) or ((gMetagameCurrentPage == 4))) then
		...
		gTopBonusIdx = random(getn(gMetaTopBonusList))

		gMetaBotBonusList = metagame_state_fnDetermineBonus(metagame_state,2)
		gBotBonusIdx = random(getn(gMetaBotBonusList))

		-- Reset clock to 15 seconds if it's been too long
		if( (gMetagameCurrentPage == 3) and (getn(gMetaTopBonusList) == 1) ) then
			metagame_state.CurTime = max(metagame_state.CurTime,gTimeOutMax)
		elseif( (gMetagameCurrentPage == 4) and (getn(gMetaBotBonusList) == 1) ) then
			metagame_state.CurTime = max(metagame_state.CurTime,gTimeOutMax)
		else
			metagame_state.CurTime = max(metagame_state.CurTime,gTimeOutMax)
		end
...
function ifs_meta_bonus_fnSetChoice(fTeam,PlanetStr)
	...
	local CurTeam         = metagame_state[format("team%d",fTeam)]
...
--
	if(gMetagameCurrentPage==1) then
		...
		if(metagame_state.CurTime < 0.01) then
			...
			local rndPlanet=random(numTiedPlanets)
			local cnt=1
			for k,v in metagame_state.planets do
				local i = v.pickorder
				local planetVotes
...
--				end
	...
	elseif((gMetagameCurrentPage==3) or (gMetagameCurrentPage==4)) then	
		if(metagame_state.CurTime < 0.01) then
			...
			local rndPlanet1 = random(numTiedPlanets1)
			local rndPlanet2 = random(numTiedPlanets2)
			local cnt1 = 1
			local cnt2 = 1
			for k,v in metagame_state.planets do
				local i = v.pickorder
...
--			if( ( getn(gMetaTopBonusList) == 1 ) and (gMetaTopBonusList[1].name == "none") ) then
			if(pickingBonus==1) then
				if( not metagame_host_state.gBonus1 ) then
...
--			end
--			if( ( getn(gMetaBotBonusList) == 1 ) and (gMetaBotBonusList[1].name == "none") ) then
				if( not metagame_host_state.gBonus2 ) then
...
function ifs_meta_main_fnUpdate(this, fDt)
	...
	if( nil and ScriptCB_InNetSession()) then
		...
		--for i = 1, getn(ifs_meta_main_listbox_contents) do
...
--		if(gMetagameCurrentPage==3) then
--			if((metagame_state.pickteam==1) and(getn(gMetaTopBonusList)==1)) then
--				metagame_vote_state.gBonus1Vote = gMetaTopBonusList[gTopBonusIdx].name
--			elseif((metagame_state.pickteam==2) and(getn(gMetaBotBonusList)==1)) then
--				metagame_vote_state.gBonus2Vote = gMetaBotBonusList[gBotBonusIdx].name
--			end
--			
--		elseif(gMetagameCurrentPage==4) then
--			if((metagame_state.pickteam==1) and(getn(gMetaBotBonusList)==1)) then
--				metagame_vote_state.gBonus2Vote = gMetaBotBonusList[gBotBonusIdx].name
--			elseif((metagame_state.pickteam==2) and(getn(gMetaTopBonusList)==1)) then
--				metagame_vote_state.gBonus1Vote = gMetaTopBonusList[gTopBonusIdx].name
--			end
--		end
--	end
...
function ifs_meta_main_fnEnter(this, bFwd)
	...
	if( bFwd ) then
		...
		--			IFText_fnSetString(Popup_DedicatedServer.title,"ifs.meta.popups.dedicated")
