	...
	Update = function(this, fDt)
		...
		if (this.timer < 0) then
			...
			elseif (this.state == 3) then  --gamespy login
				...
				if( ScriptCB_SetupAutoNetIsLan() == 1 ) then
...
--				ScriptCB_SetNetLoginName(ScriptCB_GetCurrentProfileNetName()) --only cuz it did it on enter
			...
			elseif (this.state == 5) then --ifs_mp_gameopts.lua
				...
				local iNumPlayers,iMaxPlayers,iNumBots,iMaxBots,iTeamDmg,iAutoAim,bShowNames,bAutoAssignTeams,iStartCnt,bHeros,iDifficulty = ScriptCB_GetNetGameDefaults()
				ScriptCB_SetNetGameDefaults(iNumPlayers,iNumBots,iTeamDmg,iAutoAim,0,bShowNames,bAutoAssignTeams,iStartCnt,bHeros,iDifficulty,"")
			...
			elseif (this.state == 8) then --Client Lobby update loop
				...
				if( ScriptCB_AutoNetJoin() == 1) then
