...
function ifs_meta_main_fnAddGalaxyPlanets(dest,CenterX,Width)
	...
	-- Gotta calculate max-X to make this fit
	local MaxX = 0.1
	for i = 1,NumPlanets do
		if(MaxX < abs(metagame_state.PlanetPositions[i].mapx)) then
			MaxX = abs(metagame_state.PlanetPositions[i].mapx)
		end
	end
...
ifs_meta_main = NewIFShellScreen {
	...
	nologo = 1,
    movieIntro      = nil,
    movieBackground = "ifs_meta_screen",
    music           = "shell_galacticconquest",
    
	bg_texture = HackBGTextureOnPCOnly("gc_screen"),
...
ifs_meta_main.CurButton = AddVerticalButtons(ifs_meta_main.buttons,vbutton_layout)
