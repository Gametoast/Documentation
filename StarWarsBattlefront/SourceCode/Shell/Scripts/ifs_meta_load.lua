...
function ifs_meta_load_listbox_CreateItem(layout)
	...
	Temp.NameStr = NewIFText{
		...
		textw = layout.width - 30, nocreatebackground=1, startdelay=random()*0.5, 
	}
	Temp.DateStr = NewIFText{ 
		x = layout.width * 0.5, y = 0, halign = "left", font = LineFont, 
		textw = 220, nocreatebackground=1, startdelay=random()*0.5,
	}
	return Temp
end
...
function ifs_meta_load_fnGetSavedGameList(this)
	...
	-- how many are there
	local listCount = getn(ifs_meta_load_listbox_contents)
	...
	if(listCount < 1) then
		...
		local curGame = 1
		local i
		for i = 1,getn(ifs_meta_load_listbox_contents) do
...
function ifs_meta_load_StartPromptSave()
	...
	IFText_fnSetString(Popup_LoadSave2.title,"ifs.meta.load.confirm_save")	
	-- set the button text
...
ifs_meta_load = NewIFShellScreen {
	...
	Helptext_Delete = NewIFContainer {
	...
	Enter = function(this, bFwd)
		...
		gIFShellScreenTemplate_fnMoveIcon(this.Helptext_Delete)	
	...
	Input_GeneralUp = function(this,iJoystick)
		...
		if(ifs_saveop.saveProfileNum and ifs_saveop.saveProfileNum~=iJoystick+1) then
			...
			if(gPlatformStr ~= "PC") then
				...
				if(getn(ifs_meta_load_listbox_contents)>0) then
					--IFObj_fnSetVis(this.Helptext_Delete, this.Mode=="Save" and ifs_meta_load_listbox_layout.CursorIdx>1)
				end
			end
		end
...
--	fnLoadFileListDone = ifs_meta_load_fnLoadFileListDone,
--	fnMetagameLoadDone = ifs_meta_load_fnMetagameLoadDone,
--	fnMetagameDeleteDone = ifs_meta_load_fnMetagameDeleteDone,
}
		...
		--if(this.Mode == "Save") then
			if(getn(ifs_meta_load_listbox_contents) > 0) then
				...
				if(Selection.filename) then
					...
					IFText_fnSetString(Popup_YesNo.title,"ifs.meta.load.confirm_delete")
					ifs_meta_load_SetVis(this,nil)
