...
function ifs_vkeyboard_fnCharOk(this, iKeyOffset)
	if(vkeyboard_specs.bGamespyMode) then
		if((strlen(this.WorkString) < 1) and 
...
function IFS_VKeyboard_fnCouldAddChar(this, iAscii)
	...
	local StringLen = strlen(this.WorkString)

--	print("fnCouldAdd, strlen=", StringLen, "iKeyOffset = ",iKeyOffset)

	local CurPixelLen = ScriptCB_IFText_GetTextExtent(this.buttons.DisplayString.cpointer)
	...
	if(iAscii) then
		...
		if((vkeyboard_specs.bGamespyMode) and (strlen(this.WorkString) < 1)) then
			local FirstChar = strchar(iAscii)
	...
	if((StringLen < 1) and (iKeyOffset == (13*5+0))) then
		...
		return ifs_vkeyboard_fnCharOk(this,iKeyOffset)
	end
end
...
function IFS_VKeyboard_ButtonXFromCursor(this)
	if(this.CurButton == "cancel") then
		this.CursorX = floor(vkeyboard_specs.NumColumns * 0.15)
	elseif (this.CurButton == "back") then
		this.CursorX = floor(vkeyboard_specs.NumColumns * 0.5)
	else
		this.CursorX = floor(vkeyboard_specs.NumColumns * 0.85)
	end
end
...
function IFS_VKeyboard_fnRegetKeys(this)
	...
	for i=1,vkeyboard_specs.NumRows do
		for j=1,vkeyboard_specs.NumColumns do
			...
			if(vkeyboard_specs.bGamespyMode) then
				if(not ifs_vkeyboard_fnCharOk(this, KeyOffset)) then
					ShowAlpha = 0.4
				end
			end
...
function IFS_VKeyboard_MoveCursor(this)
	...
	if(this.CursorY < vkeyboard_specs.NumRows) then
		...
		IFObj_fnSetPos(this.buttons.Cursor,
									 ...
									 vkeyboard_specs.StartY + (this.CursorY + 0.3) * vkeyboard_specs.YDiff + 4)
...
function IFS_VKeyboard_BounceCursor(this,fDt)
	...
	this.buttons.Cursor:fnSetSize(vkeyboard_specs.CursorWidth + this.ButtonAddScale,
...
function IFS_VKeyboard_ShowCurString(this)
	...
	local Len = strlen(this.WorkString)
--	print("VK, len = ", Len)
	local ShowStr = ""

	if(vkeyboard_specs.bPasswordMode) then
		ShowStr = strrep("*", strlen(this.WorkString))	
	else
	...
	local Stage = 1
	repeat
		ScriptCB_IFText_SetTextScale(this.buttons.DisplayString.cpointer,HScale,1)
	...
	if(ScriptCB_GetPlatform() ~= "PC") then
		...
		IFObj_fnSetVis(this.deletegroup,strlen(this.WorkString) > 0)
...
function IFS_VKeyboard_Backspace(this)
	local Len = strlen(this.WorkString)
	this.WorkString = strsub(this.WorkString, 1, Len - 1)
...
--				print("VKeyboard, Input_Accept, cur = 'ok'")
				...
				if(bCanAccept) then
					...
					IFText_fnSetString(Popup_Ok.title,ErrStr)
	...
	Input_LTrigger = function(this)
		if((this.CursorY < vkeyboard_specs.NumRows) and (this.CursorX > 0)) then
			this.CursorX = max(0,this.CursorX - vkeyboard_specs.HalfWidth)
	...
	Input_RTrigger = function(this)
		if((this.CursorY < vkeyboard_specs.NumRows) and (this.CursorX < (vkeyboard_specs.NumColumns - 1))) then
			this.CursorX = min(this.CursorX + vkeyboard_specs.HalfWidth,vkeyboard_specs.NumColumns - 1)
			ifelm_shellscreen_fnPlaySound(this.selectSound)
	...
	Enter = function(this, bFwd)
		...
		gIFShellScreenTemplate_fnMoveIcon(this.modegroup)

		this.WorkString = ScriptCB_ununicode(this.CurString)

		if((gPlatformStr == "PC") and (not ScriptCB_IsAutoNet())) then
--			print("Transfering to the PC keyboard")
			ScriptCB_PopScreen()
			ScriptCB_PushScreen("ifs_kb")
		end
		...
		ScriptCB_IFText_SetTextScale(this.buttons.DisplayString.cpointer,1,1)
		...
		this.buttons.DisplayStringBG:fnSetSize(ScreenW - 20, this.DisplayBoxHeight)

		if(not this.fCharOffset) then
			this.fCharOffset = 0 -- No intl characters by default
			IFS_VKeyboard_fnRegetKeys(this)
		end
...
--		AnimationMgr_AddAnimation(this.startgroup, {fStartAlpha = 0, fEndAlpha = 1,})
		...
		-- if PC hide all the help buttons
		if(ScriptCB_GetPlatform() == "PC") then
--			IFObj_fnSetVis(this.Helptext_Accept,nil)
--			IFObj_fnSetVis(this.startgroup,nil)
			IFObj_fnSetVis(this.modegroup,nil)
			IFObj_fnSetVis(this.deletegroup,nil)
		end
	end,
...
--		print("ifs_vkeyboard, KeyDown. Ascii = ",iKey)
		...
		elseif ((iKey >= 32) and (iKey < 127)) then
			if(IFS_VKeyboard_fnCouldAddChar(this, iKey)) then
				ifelm_shellscreen_fnPlaySound("shell_keyboard_key")
				this.WorkString = this.WorkString .. strchar(iKey)
				IFS_VKeyboard_ShowCurString(this)
				if(vkeyboard_specs.bGamespyMode) then
					IFS_VKeyboard_fnRegetKeys(this)
				end
...
function IFS_VKeyboard_AddButtons(this)
	...
	this.buttons = NewButtonWindow {
		ScreenRelativeX = 0.5, -- center
		ScreenRelativeY = 0.44, -- just above center
		width = ScreenW,
		height = ScreenH,
		y = ScreenH * -0.05,
		ZPos = 200,
...
--			IFText_fnSetUString(this.buttons[Offset],ScriptCB_GetVKeyboardCharacter(Offset))
	...
	vkeyboard_specs.CursorWidth = XDiff * 1.2
	vkeyboard_specs.CursorHeight = YDiff * 1.2
	...
	vkeyboard_specs.HalfWidth = floor(vkeyboard_specs.NumColumns * 0.5) 
