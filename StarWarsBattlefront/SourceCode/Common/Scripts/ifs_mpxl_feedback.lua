...
--	bVoicemailOnly = 1,
	...
	Input_Accept = function(this)
		if((this.CurButton == "session") or (this.CurButton == "attitude")) then
			...
			local i
			for i=1,getn(mpxl_feedback_vbutton_layout.buttonlist) do
			...
			IFText_fnSetUString(Popup_YesNo.title,TitleUStr)
...
function ifs_mpxl_feedback_fnBuildScreen(this)
	...
	-- Get other chunks out of lua as well
	-- Don't zap the buttonlist container (as we need to animate it thru Lua),
	-- but zap all its contents.
	local i
	for i=1,getn(mpxl_feedback_vbutton_layout.buttonlist) do
		local label = mpxl_feedback_vbutton_layout.buttonlist[i].tag
		this.buttonlabels[label].inert = 1
	end
