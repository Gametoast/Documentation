...
function AnimationMgr_AddAnimation(obj,Template)
	...
	if(Template.fStartAlpha or Template.fEndAlpha) then
		ScriptCB_IFObj_SetAlpha(obj.cpointer,obj.fStartAlpha)
	end
	...
	if(Template.fStartX or Template.fStartY) then
		ScriptCB_IFObj_SetPos(obj.cpointer,obj.fStartX,obj.fStartY)
	end
...
function AnimationMgr_Update(fDt)
	...
	while (Cur) do
		Cur.fElapsedTime = min(Cur.fElapsedTime + fDt, Cur.fTotalTime)
		local fRatio = Cur.fElapsedTime / Cur.fTotalTime

		if(Cur.fStartAlpha) then
			ScriptCB_IFObj_SetAlpha(Cur.cpointer,Cur.fStartAlpha + fRatio * (Cur.fEndAlpha - Cur.fStartAlpha))
		end
		if(Cur.fStartX) then
			ScriptCB_IFObj_SetPos(Cur.cpointer,
