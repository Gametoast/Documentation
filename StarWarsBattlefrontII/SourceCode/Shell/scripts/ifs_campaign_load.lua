...
-- fully-built item.
function ifs_campaign_load_listbox_CreateItem(layout)
	-- Make a coordinate system pegged to the top-left of where the cursor would go.
	local Temp = NewIFContainer { x = layout.x - 0.5 * layout.width, y=layout.y}

	local LineFont = ifs_campaign_load_listbox_layout.FontStr
	local FontHeight = ifs_campaign_load_listbox_layout.iFontHeight
	...
	if(gPlatformStr == "XBox") then
		Temp.NumberStr = NewIFText{ 
			...
			textw = ifs_campaign_load_listbox_layout.fNumberWidth, 
			nocreatebackground = 1, startdelay=math.random()*0.5, 
		}
		XLeft = XLeft + ifs_campaign_load_listbox_layout.fNumberWidth
	end
...
function ifs_campaign_load_listbox_PopulateItem(Dest, Data, bSelected, iColorR, iColorG, iColorB, fAlpha)
	if(Data) then
		-- Show this entry
...
ifs_campaign_load_listbox_contents = {
}

ifs_campaign_load_listbox_layout = {
	showcount = 6,
	yHeight = 34,
	ySpacing  = 0,
	width = 420,
	x = 0,
	slider = 1,
	CreateFn = ifs_campaign_load_listbox_CreateItem,
	PopulateFn = ifs_campaign_load_listbox_PopulateItem,
	fBottomArrowYOffset = -10,
	fNumberWidth = 40, -- width, in pixels, for the XBox's file #
}

function ifs_campaign_load_SetVis(this,vis)
	this.bIsVisible = vis
	...
	if( this.Helptext_Delete ) then
		IFObj_fnSetVis(this.Helptext_Delete,vis and ifs_campaign_load_fnGetShowDelete(this))
	end

--	print("ifs_campaign_load_SetVis(this,", vis, this.Mode)
...
function ifs_campaign_load_fnGetShowDelete(this)
...
--	end
	...
	if((not ifs_campaign_load_listbox_contents) or 
		 (table.getn(ifs_campaign_load_listbox_contents) < 1)) then
		return nil
	end
	...
	if(not ifs_campaign_load_listbox_layout.SelectedIdx) then
		return nil
	end

	local Selection = ifs_campaign_load_listbox_contents[ifs_campaign_load_listbox_layout.SelectedIdx]
	return not (Selection.bIsSaveAsNew)
end

function ifs_campaign_load_fnShortPopupOk()
	local this = ifs_campaign_load
	ifs_campaign_load_SetVis(this, 1)
end

function ifs_campaign_load_fnListFullOk()
	local this = ifs_campaign_load
	ifs_campaign_load_SetVis(this, 1)
end

function ifs_campaign_load_fnGetSavedGameList(this)
	-- get the saved game list from the game
	local savemode = (this.Mode == "Save")

	-- get the listbox contents	
	ifs_campaign_load_listbox_contents, this.iMaxSaves = ScriptCB_GetSavedCampaignList(savemode)
	
	-- how many are there
	local listCount = table.getn(ifs_campaign_load_listbox_contents)

	for i=1,listCount do
		ifs_campaign_load_listbox_contents[i].bIsROTE = 1
	end
	...
	if(listCount < 1) then
		...
		local curGame = 1
		local i
		for i = 1,table.getn(ifs_campaign_load_listbox_contents) do
			if(ifs_campaign_load_listbox_contents[i].isCurrent) then
				curGame = i
			end
		end
		
		-- cursor on the current (or first if no current)
		ifs_campaign_load_listbox_layout.FirstShownIdx = curGame
		ifs_campaign_load_listbox_layout.SelectedIdx = curGame
		ifs_campaign_load_listbox_layout.CursorIdx = curGame

		-- show delete?
		if( this.Helptext_Delete ) then
			IFObj_fnSetVis(this.Helptext_Delete, ifs_campaign_load_fnGetShowDelete(this))
		end
	...
	-- fill the listbox
	ListManager_fnFillContents(this.listbox,ifs_campaign_load_listbox_contents,ifs_campaign_load_listbox_layout)
...
function ifs_campaign_load_StartLoadFileList()
	ifs_saveop.doOp = "LoadFileList"
	ifs_saveop.OnSuccess = ifs_campaign_load_LoadFileListSuccess
	ifs_saveop.OnCancel = ifs_campaign_load_LoadFileListCancel
	ifs_saveop.ForceSaveFailedMessage = (ifs_campaign_load.Mode == "Save")
	ScriptCB_PushScreen("ifs_saveop")
end

function ifs_campaign_load_LoadFileListSuccess()
	-- good, continue
	print("ifs_campaign_load_LoadFileListSuccess")
	
	-- don't reload when we get back to ifs_campaign_load.Enter	
	ifs_campaign_load.bFromLoadFileList = 1
	-- pop ifs_saveop, reenter ifs_campaign_load	
	ScriptCB_PopScreen()	
end

function ifs_campaign_load_LoadFileListCancel()
	-- ok, continue
	print("ifs_campaign_load_LoadFileListCancel")
...
--	print(" Checking Mode, NoPromptSave = ", ifs_campaign_load.Mode, ifs_campaign_load.NoPromptSave)

	if((ifs_campaign_load.Mode == "Save") and (not ifs_campaign_load.NoPromptSave)) then
		-- Stay here, ask again.
		ifs_campaign_load.bFromLoadFileList = nil
		ifs_campaign_load.bErrReenter = 1
	else
...
function ifs_campaign_load_StartPromptLoad()
--	print("ifs_campaign_load_StartPromptLoad")
	local this = ifs_campaign_load

	-- hide this screen
	ifs_campaign_load_SetVis(this,nil)
	...
	Popup_LoadSave2.fnAccept = ifs_campaign_load_LoadPromptAccept	
end

function ifs_campaign_load_LoadPromptAccept(fRet)
	local this = ifs_campaign_load
	...
	if(fRet < 1.5) then
--		print("ifs_campaign_load_SavePromptAccept(A - Continue)")
...
--		print("ifs_campaign_load_SavePromptAccept(B - Cancel)")
		-- no, don't save.  pop this screen and continue
        ifelm_shellscreen_fnPlaySound(this.cancelSound)
		-- call the exit func
		if(ifs_campaign_load.ExitFunc) then
--			print("ifs_campaign_load.ExitFunc()")
			ifs_campaign_load.ExitFunc()
			ifs_campaign_load.ExitFunc = nil
		end		
		--pop it
		ScriptCB_PopScreen()
	end	
...
function ifs_campaign_load_StartLoadCampaign(name1)
	print("ifs_campaign_load_StartLoadCampaign")
	ifs_saveop.doOp = "LoadCampaign"
	ifs_saveop.OnSuccess = ifs_campaign_load_LoadCampaignSuccess
	ifs_saveop.OnCancel = ifs_campaign_load_LoadCampaignCancel
	ifs_saveop.filename1 = name1
	ScriptCB_PushScreen("ifs_saveop")
end

function ifs_campaign_load_LoadCampaignSuccess()
	print("ifs_campaign_load_LoadCampaignSuccess")
	local this = ifs_campaign_load
	
	-- don't prompt a save when we get into the campaign, since we just loaded
	ifs_campaign_main.requestSave = nil
	ifs_campaign_load.bFromLoadDelete = 1
	-- restart the shell (HACK)
	if ifs_campaign_main.wasSplit then
		ScriptCB_SetSplitscreen(ifs_campaign_main.wasSplit)
	end
	SetState("shell")
	-- 
--	if ScriptCB_IsScreenInStack("ifs_campaign_main") then
--		ScriptCB_PopScreen("ifs_campaign_main")
--	end
--	ScriptCB_PopScreen()
--	ScriptCB_PushScreen("ifs_campaign_main")
end

function ifs_campaign_load_LoadCampaignCancel()
	print("ifs_campaign_load_LoadCampaignCancel")
...
function ifs_campaign_load_StartPromptSave()
--	print("ifs_campaign_load_StartPromptSave")
	local this = ifs_campaign_load
	...
	Popup_LoadSave2.fnAccept = ifs_campaign_load_SavePromptAccept	
end

function ifs_campaign_load_SavePromptAccept(fRet)
	local this = ifs_campaign_load
	...
	if(fRet < 1.5) then
--		print("ifs_campaign_load_SavePromptAccept(A - Save)")
        ifelm_shellscreen_fnPlaySound(this.acceptSound)
		-- yes, save, so continue
		this.bPromptSave = nil
...
--		print("ifs_campaign_load_SavePromptAccept(B - Continue wihtout saving)")
		-- no, don't save.  pop this screen and continue
        ifelm_shellscreen_fnPlaySound(this.cancelSound)
		-- call the exit func
		if(ifs_campaign_load.ExitFunc) then
...
----------------------------------------------------------------------------------------
-- save the current campaign into the slot selected
----------------------------------------------------------------------------------------

function ifs_campaign_load_StartSaveCampaign(aOldFilename, aNewFilename)
-- 	print("ifs_campaign_load_StartSaveCampaign()")
-- 	if(aOldFilename) then
-- 		print(" OldName = ", ScriptCB_ununicode(aOldFilename))
-- 	end
-- 	if(aNewFilename) then
-- 		print(" NewName = ", ScriptCB_ununicode(aNewFilename))
-- 	end
	
	-- save current state	
	ifs_campaign_main:SaveState()
	
	ifs_saveop.doOp = "SaveCampaign"
	ifs_saveop.OnSuccess = ifs_campaign_load_SaveCampaignSuccess
	ifs_saveop.OnCancel = ifs_campaign_load_SaveCampaignCancel
...
function ifs_campaign_load_SaveCampaignSuccess()
	print("ifs_campaign_load_SaveCampaignSuccess")
	-- pop ifs_saveop
	ifs_campaign_load.PopOnEnter = 1
	ScriptCB_PopScreen()
end

function ifs_campaign_load_SaveCampaignCancel()
	print("ifs_campaign_load_SaveCampaignCancel")
	-- do nothing when we reenter ifs_campaign_load
	ifs_campaign_load.bFromSaveCancel = 1
	ifs_campaign_load.bFromDeleteCancel = 1
	-- bail from ifs_saveop
	ScriptCB_PopScreen()
	-- don't pop to campaign, since we might want to try again
end
...
function ifs_campaign_load_fnDeletePopupDone(bResult)
	local this = ifs_campaign_load
	
	if(bResult) then
--		print("ifs_campaign_load_fnDeletePopupDone(true)")
		ifs_campaign_load_SetVis(this, 1)

		ifelm_shellscreen_fnPlaySound(this.acceptSound)
		-- User does want to delete
		local Selection = ifs_campaign_load_listbox_contents[ifs_campaign_load_listbox_layout.SelectedIdx]
--		print("idx = ",ifs_campaign_load_listbox_layout.SelectedIdx)
		
		ifs_saveop.doOp = "DeleteCampaign"
		ifs_saveop.OnSuccess = ifs_campaign_load_DeleteCampaignSuccess
		ifs_saveop.OnCancel = ifs_campaign_load_DeleteCampaignCancel
		ifs_saveop.filename1 = Selection.filename
		ScriptCB_PushScreen("ifs_saveop")
		
	else
        ifelm_shellscreen_fnPlaySound(this.cancelSound)
--		print("ifs_campaign_load_fnDeletePopupDone(false)")
		-- User hit no. Back to normal screen
		ifs_campaign_load_SetVis(this, 1)
	end
end

function ifs_campaign_load_DeleteCampaignSuccess()
--	print("ifs_campaign_load_DeleteCampaignSuccess")
	ifs_campaign_load.bFromLoadDelete = 1
	ScriptCB_PopScreen()
end

function ifs_campaign_load_DeleteCampaignCancel()
--	print("ifs_campaign_load_DeleteCampaignCancel")
	ifs_campaign_load.bFromLoadDelete = 1
	ifs_campaign_load.bFromDeleteCancel = 1
	-- bail from ifs_saveop
	ScriptCB_PopScreen()
...
ifs_campaign_load = NewIFShellScreen {
	nologo = 1,
	movieIntro      = nil,
	movieBackground = nil,
...
--		print("ifs_campaign_load.Enter(",bFwd,")")
		gIFShellScreenTemplate_fnEnter(this, bFwd)
		ifs_campaign_load_SetVis(this,1) -- default, turn things on on entry
		...
		if(this.bSaveOnEnter) then
			...
			ifs_campaign_load_StartSaveCampaign(this.LastFilenameUStr, this.NewFilenameUStr)
			return
		end
...
--			print("ifs_campaign_load.PopOnEnter")
			this.PopOnEnter = nil
			-- call the exit func
			if(ifs_campaign_load.ExitFunc) then
--				print("ifs_campaign_load.ExitFunc()")
			...
			-- pop ifs_campaign_load, return to campaign
			ScriptCB_PopScreen()
			return
		end
		...
		ifs_campaign_load.bErrReenter = nil -- always clear this
...
--			print("ifs_campaign_load.Enter Load")
			...
			if(not (this.NoPromptSave or this.SkipPromptSave or this.bFromLoadFileList or this.bFromLoadDelete or this.bFromSaveCancel)) then
				ifs_campaign_load_StartPromptLoad()
				return
			end
...
--			print("ifs_campaign_load.Enter Save", (not this.NoPromptSave) , not this.bFromLoadFileList, not this.bFromLoadDelete , not this.bFromSaveCancel)
			
			-- if this is the first time entering, prompt for "do you want to save yes/no?"
			if(not (this.NoPromptSave or this.SkipPromptSave or this.bFromLoadFileList or this.bFromLoadDelete or this.bFromSaveCancel)) then
				ifs_campaign_load_StartPromptSave()
				return
			end
			this.NoPromptSave = nil
			...
			print("ifs_campaign_load.Mode not set (either Load or Save)")
			assert(ifs_campaign_load.Mode == "Load" or ifs_campaign_load.Mode == "Save")
		end
		...
		if(bFwd or this.bFromDeleteCancel) then
			...
			if(not this.bFromLoadFileList) then
				ifs_campaign_load_StartLoadFileList()
			end
			this.bFromLoadFileList = nil
		else
			ifs_campaign_load_fnGetSavedGameList(ifs_campaign_load)		
		end
 	...
 	Exit = function(this, bFwd)
 		...
 		ifs_campaign_load_listbox_contents = nil
 		this.bFromLoadFileList = nil
	...
	Input_GeneralUp = function(this,iJoystick)
		...
		if(not gCurEditbox) then
			ListManager_fnNavUp(this.listbox,ifs_campaign_load_listbox_contents,ifs_campaign_load_listbox_layout)
		end
	...
	Input_LTrigger = function(this,iJoystick)
		if(not gCurEditbox) then
			ListManager_fnPageUp(this.listbox,ifs_campaign_load_listbox_contents,ifs_campaign_load_listbox_layout)
		end
	...
	Input_GeneralDown = function(this,iJoystick)
		...
		if(not gCurEditbox) then
			ListManager_fnNavDown(this.listbox,ifs_campaign_load_listbox_contents,ifs_campaign_load_listbox_layout)
		end
	...
	Input_RTrigger = function(this,iJoystick)
		if(not gCurEditbox) then
			ListManager_fnPageDown(this.listbox,ifs_campaign_load_listbox_contents,ifs_campaign_load_listbox_layout)
		end
		IFObj_fnSetVis(this.Helptext_Delete,ifs_campaign_load_fnGetShowDelete(this))
	end,
		...
		if(gPlatformStr == "PC") then
			...
			if(this.Mode == "Load") then
				...
				if(gCurEditbox and gCurEditbox.bKeepsFocus) then
					...
					ifs_campaign_load_listbox_layout.CursorIdx = nil
					ifs_campaign_load_listbox_layout.SelectedIdx = nil
					ListManager_fnFillContents(this.listbox, ifs_campaign_load_listbox_contents, ifs_campaign_load_listbox_layout)
				else
					this.nameedit.bKeepsFocus = nil
					RoundIFButtonLabel_fnSetString(this.Helptext_Accept,"ifs.meta.load.btnsave")
					IFObj_fnSetVis(this.Helptext_Accept, this.bIsVisible and ifs_campaign_load_listbox_contents and 
												 table.getn(ifs_campaign_load_listbox_contents) > 0)
				end
			end
...
--				print("+++this.CurButton = ", this.CurButton)
				ifs_campaign_load_DeleteGame(this,iJoystick)
				return
		...
		if(this.Mode == "Load") then
			...
			if(table.getn(ifs_campaign_load_listbox_contents) > 0) then
				-- load the campaign game data from file
				local Selection = ifs_campaign_load_listbox_contents[ifs_campaign_load_listbox_layout.SelectedIdx]
				ifelm_shellscreen_fnPlaySound(this.acceptSound)
				ifs_campaign_load_StartLoadCampaign(Selection.filename)
			end
		...
		elseif (this.Mode == "Save") then
			...
			if(EntryEditbox) then
				...
				if(string.len(CurString) < 1) then
					ifs_campaign_load_SetVis(this, nil)
					Popup_Ok.fnDone = ifs_campaign_load_fnShortPopupOk
			...
			if(Selection and (Selection.bIsSaveAsNew) and
				 (table.getn(ifs_campaign_load_listbox_contents) > this.iMaxSaves)) then
				ifs_campaign_load_SetVis(this, nil)
				Popup_Ok_Large.fnDone = ifs_campaign_load_fnListFullOk
...
--				print("Starting save!")
				...
				ifs_campaign_load_StartSaveCampaign(Selection.filename, Selection.namestr)
			else
				...
				vkeyboard_specs.fnDone = ifs_campaign_fnKeyboardDone
				vkeyboard_specs.fnIsOk = ifs_campaign_fnIsAcceptable
...
--		print("ifs_campaign_load.Input_Back()")
	...
	Input_Misc = function(this,iJoystick)
		if(gPlatformStr ~= "PC") then
			ifs_campaign_load_DeleteGame(this,iJoystick) -- call helper function
		end
	end,
...
function ifs_campaign_load_DeleteGame(this,iJoystick)
	-- no delete on load
	if((gPlatformStr == "PC") or (this.Mode == "Save")) then
		if(table.getn(ifs_campaign_load_listbox_contents) > 0) then
			...
			if(Selection.filename) then
				...
				Popup_YesNo.fnDone = ifs_campaign_load_fnDeletePopupDone
				ifs_campaign_load_SetVis(this,nil)
...
function ifs_campaign_load_fnBuildScreen(this)
	...
	ifs_campaign_load_listbox_layout.FontStr = "gamefont_small"
	ifs_campaign_load_listbox_layout.iFontHeight = ScriptCB_GetFontHeight(ifs_campaign_load_listbox_layout.FontStr)
	ifs_campaign_load_listbox_layout.yHeight = (2 * ifs_campaign_load_listbox_layout.iFontHeight) + 4

	local RowHeight = ifs_campaign_load_listbox_layout.yHeight + ifs_campaign_load_listbox_layout.ySpacing
	ifs_campaign_load_listbox_layout.showcount = math.floor(listHeight / RowHeight)
	...
	this.listbox = NewButtonWindow { 
		...
		y = listYPos,
		width = listWidth,
		height = ifs_campaign_load_listbox_layout.showcount * RowHeight + 30,
		titleText = "ifs.meta.load.load"
	}
	ifs_campaign_load_listbox_layout.width = listWidth - 40
	ifs_campaign_load_listbox_layout.x = 0

	if(gPlatformStr == "XBox") then
		ifs_campaign_load_listbox_layout.fXCursorOffset = ifs_campaign_load_listbox_layout.fNumberWidth
	end

	ListManager_fnInitList(this.listbox, ifs_campaign_load_listbox_layout)
...
ifs_campaign_load_fnBuildScreen(ifs_campaign_load)
ifs_campaign_load_fnBuildScreen = nil
AddIFScreen(ifs_campaign_load,"ifs_campaign_load")
