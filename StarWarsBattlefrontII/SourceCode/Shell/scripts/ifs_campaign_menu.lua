...
ifscampaignmenu_vbutton_layout = {
	ySpacing = 5,
	width = 260,
...
function ifs_campaign_menu_fnSetPieceVis(this, bVis)
	IFObj_fnSetVis(this.buttons,bVis)
end

-- Callback for when the "really quit?" popup is over.  If bResult is
-- true, user wanted to quit
function ifs_campaign_menu_fnQuitPopupDone(bResult)
	local this = Popup_YesNo.calledFrom
	ifs_campaign_menu_fnSetPieceVis(this,1) -- always restore screen
	...
	if(bResult) then
		...
		ifs_campaign_main:PromptSave(true)
	else
...
-- Callback for when the "really quit?" popup is over.  If bResult is
-- true, user wanted to quit
function ifs_campaign_menu_fnExitPopupDone(bResult)
	local this = Popup_YesNo.calledFrom
	ifs_campaign_menu_fnSetPieceVis(this,1) -- always restore screen
...
function ifs_campaign_menu_fnEnter(this, bFwd)
	...
	this.CurButton = ShowHideVerticalButtons(this.buttons,ifscampaignmenu_vbutton_layout)
	...
	if(bShowFriends) then
		...
		IFObj_fnSetVis(this.buttons.FriendIcon, 1) -- just hide it.
	else
...
function ifs_campaign_menu_fnInput_Accept(this)
	...
	elseif (this.CurButton == "quit") then
		...
		Popup_YesNo.fnDone = ifs_campaign_menu_fnQuitPopupDone
		...
		ifs_campaign_menu_fnSetPieceVis(this, nil)
	elseif (this.CurButton == "exit") then
		Popup_YesNo.calledFrom = this
		Popup_YesNo.CurButton = "no" -- default
		Popup_YesNo.fnDone = ifs_campaign_menu_fnExitPopupDone
		ifs_campaign_menu_fnSetPieceVis(this, nil)
		Popup_YesNo:fnActivate(1)
	...
	elseif (this.CurButton == "cheat") then
		ifs_freeform_cheat.main = ifs_campaign_main
...
ifs_campaign_menu = NewIFShellScreen {
	nologo = 1,
	bAcceptIsSelect = 1,
	...
	Enter = function(this, bFwd)
		ifs_campaign_menu_fnEnter(this,bFwd) -- Call function shared between instances
	end,

	Input_Accept = function(this, joystick)
		ifs_campaign_menu_fnInput_Accept(this) -- Call function shared between instances
	end,
...
ifs_campaign_menu.Viewport = 0
ifs_campaign_menu.CurButton = AddVerticalButtons(ifs_campaign_menu.buttons,ifscampaignmenu_vbutton_layout)
AddIFScreen(ifs_campaign_menu,"ifs_campaign_menu", 1)

ifs_campaign_menu = DoPostDelete(ifs_campaign_menu)
